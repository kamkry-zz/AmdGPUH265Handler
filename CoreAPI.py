...
for track in media_info.tracks:
    if track.track_type == 'Video':                             # Preapring info about current video bitrate
      break
...
if str(confirmation) == "yes":
    ...
    command = Path("ffmpeg -hwaccel auto -i " + str(path_input) + "/" + str(file) + " -c:v hevc_amf -preset medium -b:v " + str(target_bitrate_video) + "M -c:a aac -b:a " + str(target_bitrate_audio) + "k " + str(path_input) + "/" + r"_h265/" + str(file))
